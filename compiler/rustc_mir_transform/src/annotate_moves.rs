...
impl AnnotateMoves {
    ...
    /// Used for Call arguments where we need to store SourceInfo separately.
    /// Returns None if the operand should not be annotated.
                ...
                loop {
                    ...
                    if let Some((instance, _)) = scope_data.inlined {
                        ...
                        if let Some(def_id) = instance.def_id().as_local() {
                            ...
                            let is_compiler_move = self.compiler_move == Some(def_id);
                            let is_compiler_copy = self.compiler_copy == Some(def_id);
